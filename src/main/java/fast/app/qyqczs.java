...
public class qyqczs extends Super {
	
	public String init(Map<String, Object> rmap) {
		try {
			// 初始化 bs（数据库访问）、request（请求）,response（响应）,par（layui,table自定义参数）
			initMap(rmap);
			return "qyqrqc/qyqczs";
		} catch (Exception e) {
			e.printStackTrace();
			return "qyqrqc/qyqczs";
		}
	...
	public String queryQyqc(Map<String, Object> rmap) {
		try {
			...
			String querysql = "";
				...
				if(StringUtils.isNotBlank(ssjd)) {
					querysql += " and s.ssjd = '" + ssjd + "' ";
				}
				querysql +=  " order by s.zszt,s.status DESC,s.fbrq desc";
	...
	public String saveQyqczs(Map<String, Object> rmap) {
		try {
			...
			String updatesql = "update fast_qyqc f set  f.ZSZT = 'Y' where id = '" + id + "'";
			System.out.println(updatesql);
			this.getBs().insert(updatesql);
