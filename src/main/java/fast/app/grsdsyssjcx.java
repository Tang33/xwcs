...
public class grsdsyssjcx extends Super{
	...
	//查询分组
		 public String queryfz(Map<String, Object> rmap) {
			  try {
			   initMap(rmap);
			   String sql = "SELECT DISTINCT mbm,zdm FROM FAST_YSSJCX_MB where DYCXB = 'xwcs_gsdr_yssjgs'  ";
	...
	public String queryzd(Map<String, Object> rmap) {
		try {
			...
			String sql="select "+xz+" xmmc from xwcs_gsdr_yssjgs group by "+xz;
	...
	public String selectList(Map<String, Object> rmap) {
		...
		try {
			...
			String sql = "select MBM,ZDM FROM  FAST_YSSJCX_MB where DYCXB = 'xwcs_gsdr_yssjgs' group by MBM,ZDM  ";
			List<Map<String,Object>> list = this.getBs().query(sql);
	...
	public String selectislike(Map<String, Object> rmap) {
		try {
			...
			String sql = "select * from fast_yssjcx_mb where ZDM = '" + zdm + "' and DYCXB = 'xwcs_gsdr_yssjgs'";
			List<Map<String,Object>> list = this.getBs().query(sql);
	...
	//查询多选框
	 public String selectSZ(Map<String, Object> rmap) {
	  try {
	   initMap(rmap);
	   String xz = getValue(this.getForm().get("xz"));
	   String sql =  "select DISTINCT "+xz+" selname from XWCS_GSDR_YSSJGS";
	   List<Map<String,Object>> list = this.getBs().query(sql);
