 ...
 *
 */
public class qyhzb2 extends Super {
...
//			sql="SELECT * FROM TEMP_QUERYYDHZBBYQY_MX2";
...
//			map.put("sjlist", sjlist);
	 ...
	 * @return
	 */
	public String queryHzb2(Map<String, Object> rmap){
		try{
			//初始化         bs（数据库访问）、request（请求）,response（响应）,par（layui,table自定义参数）
			initMap(rmap);
...
//			将年月拆开
			...
			if ("".equals(pageNo)) {
				...
				if (count.equals("0")) {
					...
					List<Map<String, Object>> rsjList= (List<Map<String, Object>>) JdbcConnectedPro.call("PKG_CX_WBW.QUERYYDHZBBYQY_FSZ_FB2", list);
			...
			list.add(new Mode("IN","String",tjdw));
			list.add(new Mode("OUT","RS",""));
			List<Map<String, Object>> sjList= (List<Map<String, Object>>) JdbcConnectedPro.call("PKG_CX_WBW.QUERYYDHZBBYQY_FSZ_FB2", list);
...
//				将年月拆开
				...
				String[] cols = { "纳税人名称", "本年累计(营业收入情况)", "上年同期(营业收入情况)", "增减率(营业收入情况)", "上年累计(营业收入情况)"
						, "本年累计(税收情况)", "上年同期(税收情况)", "增减率(税收情况)", "上年累计(税收情况)", "行业", "经济类型", "所属街道"};
				String[] keys = { "NSRMC", "YSSRBYLJ", "YSSRSNTQS", "YSSRTBZJE", "YYSRSNQNHJ", "SSBNYLJ", "SNSSTQS", "SSTBZJL", "SSSNQNHJ" 
						,"HY_MC","JJLX","JD_MC"};
