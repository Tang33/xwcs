 ...
 *
 */
public class jbqkfszhz extends Super {

	public String init(Map<String, Object> rmap) {
		try {
			// 初始化 bs（数据库访问）、request（请求）,response（响应）,par（layui,table自定义参数）
			initMap(rmap);
			return "wbw/jbqkfszhz";
		} catch (Exception e) {
			e.printStackTrace();
			return "wbw/jbqkfszhz";
		}
...
//			
//			sql="SELECT * FROM TEMP_QUERYYDHZBBYQY_FSZ";
...
//			map.put("sjlist", sjlist);
	 ...
	 * @return
	 */
	public String queryFszHzb(Map<String, Object> rmap) {
		try {
			// 初始化 bs（数据库访问）、request（请求）,response（响应）,par（layui,table自定义参数）
			initMap(rmap);
...
//			将年月拆开
			...
			if ("".equals(pageNo)) {
				...
				if (count.equals("0")) {
					...
					list1.add(new Mode("IN", "String", zsxm_dm));
					list1.add(new Mode("IN", "String", "1"));
					...
					List<Map<String, Object>> rsjList = (List<Map<String, Object>>) JdbcConnectedPro
							.call("PKG_CX_WBW.QUERYYDHZBBYQY_FSZ", list1);
			...
			List<Map<String, Object>> sjList = (List<Map<String, Object>>) JdbcConnectedPro
					.call("PKG_CX_WBW.QUERYYDHZBBYQY_FSZ", list);
...
//					将年月拆开
			...
			String[] cols = { "税种", "本年截止当月税收", "上年同期数", "同比增减额", "同比增减率" };
			String[] keys = { "ZSXM_MC", "SSBNYLJ", "SNSSTQS", "SSTBZJE", "SSTBZJL" };
