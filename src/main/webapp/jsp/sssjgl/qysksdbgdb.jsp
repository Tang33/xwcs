          ...
          <div class="layui-row" style="margin-top: 20px">
						...
						<div class="layui-col-md3" style="text-align: right; margin-left: 110px">
							<div class="layui-inline">
								...
								<!-- <div class="layui-btn-group">
									<button class="layui-btn layui-btn-normal" id="plgg"
										type="button" lay-submit="" lay-filter="plgg">批量更改属地</button>
								</div> -->
								<div class="layui-btn-group">
	                  				<button class="layui-btn layui-btn-normal" id="plcf" type="button" lay-submit="" lay-filter="plcf">批量拆分金额</button>
                				</div>
                				<div class="layui-btn-group">
	                  				<button class="layui-btn layui-btn-normal" id="qbcf" type="button" lay-submit="" lay-filter="qbcf">全部拆分金额</button>
                				</div>
							</div>
						</div>
					</div>
	...
	<script>
		...
		layui.use([ 'form', 'layedit', 'laydate' ], function() {
			...
			form.on('submit(button)', function() {
				...
				loadlayer = layer.msg('正在处理，请稍候……', { icon: 16, shade: 0.3,shadeClose:false,time:60000 });
      				layui.use('table', function(){
    					 ...
    					 table.render({
    						...
    						limit: 10,
    							...
    							console.log("高度："+$(".layui-table-box").css("height"));
			...
			form.on('submit(formQB)', function(obj) {
				...
				loadlayer = layer.msg('正在读取数据，请稍候……', { icon: 16, shade: 0.01,shadeClose:false,time:60000 });
				var sz = new Array();
			...
			form.on('submit(plcf)', function(obj) {
		        ...
		        s += '</tr>';
		        s += '<tr>';
		        s += '<td colspan="3" style="text-align:center;">拆分后税款信息</td>';
		        s += '<td style="text-align: center;"><button class="layui-btn layui-btn-xs" id="add" type="button" lay-submit="" lay-filter="add">添加</button></td>';
		        ...
		        var k = "";
				k += '<tr>';
				k += '<td colspan="4" style="text-align: center;"><button class="layui-btn layui-btn-xs" id="save" type="button" lay-submit="" lay-filter="plsave">保存</button></td>';
				k += '</tr>';
				$("#insertbody2").html(k);
			...
			form.on('submit(plsave)', function() {
		        ...
		        arraystr+="]";
		        var cxrq = $("#test8").val();
		        console.log("111111111111111111"+xhs);
		        loadlayer = layer.msg('正在处理读取数据，请稍候……', { icon: 16, shade: 0.3,shadeClose:false,time:60000 });
		        ajax({
		        	url : "dh/CF.do",
		          	data : {
								str : arraystr,
		...
		function getData(nsrmc) {
			...
			loadlayer = layer.msg('正在处理，请稍候……', { icon: 16, shade: 0.3,shadeClose:false,time:60000 });
			wait = layer.msg('正在加载数据，请稍候……', { icon: 16, shade: 0.3,shadeClose:false,time:60000 });
        ...
        $("#inserttable").hide();
				...
				layui.table.on('tool(user)', function(obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
					...
					if (layEvent === 'cf') { //删除
						...
						var t = "";
						t += '<tr>';
						t += '<td colspan="4" style="text-align: center;"><button class="layui-btn layui-btn-xs" id="save" type="button" lay-submit="" lay-filter="save">保存</button></td>';
						t += '</tr>';
						$("#insertbody2").html(t);
