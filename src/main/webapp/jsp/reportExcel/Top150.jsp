	...
	<form class="layui-form" id="form1" action="">
		<div class="layui-form" lay-filter="test1">
			...
			<fieldset class="layui-elem-field layui-field-title"
				...
				<legend style="font-size: 12px;">单位： </legend>
				<table class="layui-table" id="table" lay-filter="user">
					<thead>
						<tr>
							...
							<th lay-data="{field:'QN',align:'right'}">去年</th>
							<th lay-data="{field:'JN',align:'right'}">今年</th>
							<th lay-data="{field:'TQ',align:'right'}">今年同期</th>	
							<th lay-data="{field:'ZB',align:'right'}">直比增减</th>	
						</tr>
					</thead>
					<tbody id="ttbody">
...
<script>
		...
		layui.use(['form', 'layedit', 'laydate','laypage'], function() {
			...
			form.on('submit(exportExcel)', function(data) {
				wait = layer.load();
				window.location.href="top150/export.do?"+$("#form1").serialize();
				layer.close(wait);
		...
		function getData() {	
			wait = layer.load();
			ajax({
				url : "top150/queryData.do?pageNo=" + pageNo + "&pageSize="+ pageSize,
				page:true,
				data :{			
					date: $("#yearNmonth").val(),
				},
