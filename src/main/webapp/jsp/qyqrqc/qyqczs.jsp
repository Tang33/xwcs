...
<body style="overflow-x: hidden">
		...
		<form class="layui-form" id="form" action="">
					...
					<div class="layui-colla-content  layui-show">
						<div class="layui-row">
                			...
                			<div class="layui-col-md3">
								...
								<div class="layui-input-inline">
									<select id="ssjd" name="ssjd" lay-filter="ssjd" >
		 ...
		 {{# if(d.STATUS == "1"&&d.ZSZT=="N"){ }}
  				  <a class="layui-btn layui-btn-normal layui-btn-sm" lay-event="ZS">终审</a>
          {{# } }}	
		...
		<!-- {{# if(d.STATUS == "1" && d.ZSZT=="Y"){ }}
  				  <a class="layui-btn layui-btn-normal layui-btn-sm" lay-event="checkDetail">任务详情</a>
          {{# } }} -->
    ...
    layui.use([ 'form', 'laydate','table' ], function() {
      ...
      form.on('submit(search_qy_btn)', function() {
    	  ...
    	  ajax({
          url : "ajax.do?ctrl=qyqczs_queryQymc",
          data : {
      ...
      form.on('submit(check_save_btn)', function(obj) {
        ajax({
          url : "ajax.do?ctrl=qyqczs_checkQyqc",
          data : obj.field,
          type : 'post',
          success : function(obj) {
            console.log(obj);
      ...
      layui.use('table', function(){
        ...
        table.render({
          ...
          ,cols: [[ //表头
            ...
            ,{field: 'FBRQ', title: '发布日期', width: 180, templet: function(data) {
            	 var fbrq='';
            ...
            ,{field: 'STATUS', title: '状态', width: 170, templet: function(data) {
            	console.log(data.STATUS);
            	if(data.STATUS == '0'&& data.ZSZT == 'N') {
            		return "待审核";
            	} else if(data.STATUS == '1'&& data.ZSZT == 'N'){
            		return "已审核";
            	}else if(data.STATUS == '1'&& data.ZSZT == 'Y'){
            		return "已终审";
            	}
            }}
             ,{field : '', title : '操作', toolbar : '#bar', fixed : 'right', width:350} 
          ]]
        });
        ...
        var num = 0;
         layui.table.on('row(qrtable)', function(obj) {
        	  ...
        	  var fkrq='';
        	  if(obj.data.FKRQ!=undefined&obj.data.FKRQ!="undefined"&obj.data.FKRQ!=null){
        		  fkrq=obj.data.FKRQ.substring(0, 10);
        	  }
        	  var fbrq='';
        	  if(obj.data.FBRQ!=undefined&obj.data.FBRQ!="undefined"&obj.data.FBRQ!=null){
        		  fbrq=obj.data.FBRQ.substring(0, 10);
        	  }
        	  
        	  $('#NSRSBH-span').text(obj.data.NSRSBH);
        	  $('#NSRMC-span').text(obj.data.NSRMC);
        	  $('#SSJD-span').text(obj.data.JDMC);
        	  $('#QCYY-span').text(obj.data.QCYY);
        	  $('#NQRD-span').text(obj.data.NQRD);
        	  ...
        	  if(typeof(obj.data.SYSSGXBL)!='undefined'&&typeof(obj.data.SYSSGXBL)!=''&&typeof(obj.data.SYSSGXBL)!=undefined){
        		 
        		  $('#SYSSGXBL').text(obj.data.SYSSGXBL+"%");
        		}
        	  $('#ZCZB-span').text(obj.data.ZCZB);
        	  $('#ZCDZ-span').text(obj.data.ZCDZ);
      ...
      layui.table.on('tool(qrtable)', function(obj) {
          ...
          if(layEvent === 'remove') {
	          ...
	          ,content: '<div style="font-size: 24px;padding-top: 5%;padding-left: 5%;">确定删除该条企业迁出记录?</div>'
	        	  ...
	        	  ajax({
		            url : "ajax.do?ctrl=qyqczs_removeQyqc",
		            data : {
		              "id" : obj.data.ID
		            },
		            type : 'post',
              ...
              ,function(index) {
                ...
                ajax({
		            url : "ajax.do?ctrl=qyqczs_saveQyqczs",
		            data : {
		              "id" : obj.data.ID	
		            },
		            type : 'post',
