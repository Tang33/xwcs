...
<script>
	...
	function getMaxData() {
		...
		async : true, //是否异步
		//url : "ajax.do?ctrl=Sjdr_getNsDate",
		url : "JDreport/JdSswcqkLrAction_jdInit.do",
		data : "",
		dataType : "json",
		success : function(obj) {
			var data = obj.data
	...
	function getData(m) {
		...
		$.ajax({
			...
			url : "JDreport/JdSswcqkLrAction_doQuery.do?pageNo=" + pageNo+ "&pageSize=" + pageSize,
			data : $("#form").serialize(),
			dataType : "json",
			success : function(obj) {
					...
					choose: function(obj){
						...
						if(res.mesg=='上传成功'){	
							$.ajax({
								...
								url:"JDreport/JdSswcqkLrAction_addBB.do",
								data: { 
				...
				layer.confirm('',{
					...
					content : '<div style="font-size: 24px;padding-top: 5%;padding-left: 5%;">真的删除'+ data.BBMC+ '么?</div>'
						...
						$.ajax({
							...
							url : "JDreport/JdSswcqkLrAction_doDel.do",
							data : data,
							dataType : "json",
							success : function(
									obj) {
