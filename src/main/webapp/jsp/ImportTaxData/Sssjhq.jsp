						...
						<div class="layui-col-md6" >
							...
							<div class="layui-inline">
								...
								<button type="button" class="layui-btn" id="doinput">
									<i class="layui-icon" style="vertical-align:middle"></i>上传比对
								</button>
							</div>
						</div>
...
<script>
	...
	//判断要导入月的数据是否加工过
	function queryZqyIs(value){
		var indexv = layer.load();		
			$.ajax({
				type : "post", //请求方式
				async : true, //是否异步
				url:"sssjhq/isjgg.do",
				...
				success:function(obj) {
					layer.close(indexv);
					if(obj.code == "000"){
						$("#doinput").addClass("layui-btn-disabled");
						$("#doinput").attr("disabled",true);
						layer.confirm('当前选择的月份数据已经加工过，不能在导入了，请重新选择入库日期', {icon: 1,
							shade: [0.8, '#393D49'],btn: ['确定'] //按钮
							}, function(){
								layer.closeAll();
							});
					}else{
						$("#doinput").removeClass("layui-btn-disabled");
						$("#doinput").attr("disabled",false);
					}
				}
			})
	...
	layui.use('upload',function() {
						...
						uploadListIns=upload.render({
									elem : '#select',
									multiple: true,
									url : 'upload.do?lx=rksj&name=' + $("#rkrq").val(),
									before : function(obj) {
										wait = layer.load();
									},
									accept : 'file' //普通文件
										...
										if (res.code == "0") {
											...
											$
													.ajax({
														type : "post", //请求方式
														async : true, //是否异步
														url : "sssjhq/doInputnew.do",
														data : {
															filenames : filenames,
															drfs : drfs,
															an : $('input:radio:checked').val(),
															rkrq : $('#rkrq').val()
															},
														dataType : "json",
														timeout: 1000*60*30,
														success : function(obj) {
															if (obj.code == "000") {
																...
																layer.alert("本月数据已经加工过",{icon: 1},function () {
																    layer.close(layer.index);
																    window.location.reload(); 
													            }); 
															}else{
	...
	function getData() {
		...
		$.ajax({
			type : "post", //请求方式
			async : false, //是否异步
			url : "sssjhq/query1.do?pageNo=" + pageNo + "&pageSize="
					+ pageSize,
			data : $("#form").serialize(),
			dataType : "json",
			success : function(obj) {
	...
	function getTbale(data) {
		...
		$.each(data, function(v, o) {
			...
			s += '<td>' + (o.WJDRSL == undefined ? "-" : o.WJDRSL) + '</td>';
			s += '<td>' + (o.QCSL == undefined ? "-" : o.QCSL) + '</td>';
			s += '<td>' + (o.DRSJ == undefined ? "-" : o.DRSJ) + '</td>';
			s += '<td>' + (o.RK_RQ == undefined ? "-" : o.RK_RQ) + '</td>';
			s += '<td>' + (o.BZ == undefined ? "-" : o.BZ) + '</td></tr>';
		});
		$("#ttbody").html(s);
		//执行渲染
		...
		form.on('submit(formDemo)', function(data) {
			...
			if (tjfx == "0") {
				$.ajax({
					type : "post", //请求方式
					async : true, //是否异步
					url : "sssjhq/Update.do&nsrmc=" + nsrmc + "&jddm=" + jd
							+ "&jdmc=" + jdmc,
					data : $("#form").serialize(),
					dataType : "json",
					success : function(obj) {
				...
				$.ajax({
					type : "post", //请求方式
					async : true, //是否异步
					url : "sssjhq/UpdateAll.do&nsrmc=" + nsrmc + "&jddm=" + jd
							+ "&jdmc=" + jdmc,
					data : $("#form").serialize(),
					dataType : "json",
					success : function(obj) {
