	...
	function queryinit(){
		layui.use('table', function(){
	        ...
	        table.render({
	          ...
	          ,where: {
	        	  data : $("#form").serialize(),
	        	  paramNsrmc : $("#paramNsrmc").val()
		       }
	          //开启分页
	          ,cols: [[ //表头
	        	  {field:'NSRSBH',title: '纳税人识别码 ',width:'20%',fixed: 'left'}, 
	        	  {field:'NSRMC',title: '纳税人名称 ',width:'30%',fixed: 'left'},
	        	  {field:'SCJYDZ',title: '生产经营地址 ',width:'20%',fixed: 'left'},
	        	  {field:'RQ',title: '最后变更日期 ',width:'20%',fixed: 'left'},
	        	  {field:'dfkj',title: '操作 ',width:'10%',fixed: 'left',toolbar:'#bar'},
...
/* 	function formatDateTime(inputTime) {  
	...
	layui.use([ 'form' ], function() {
		...
		form.on('submit(doExcel)', function(data) {
			...
			window.location.href="xzqyqcgl/exportExcel.do?paramNsrmc="+paramNsrmc;
			pageNo = 1; //当点击搜索的时候，应该回到第一页
			return false; //阻止表单跳转。如果需要表单跳转，去掉这段即可。
		});
		...
		function deleteAndInsertAjax(){
			$.ajax({
				type : "post", //请求方式
				async : false, //是否异步
				url : "xzqyqcgl/doDeleteAndInsert.do",
				data : $("#form3").serialize(),
				dataType : "json",
				success : function(obj) {
					if (obj != null && obj.code == '000') {
