...
<body style="overflow-x: hidden">
		...
		<form class="layui-form" id="form" action="">
			...
			<fieldset class="layui-elem-field layui-field-title" style="margin-top: 20px;">
				...
				<table class="layui-table" id="table" lay-filter="sj">
					<thead>
						<tr>
							<th lay-data="{field:'NSRSBH',width:'15%'}">纳税人识别码</th>
							<th lay-data="{field:'NSRMC',width:'20%'}">纳税人名称</th>
							<th lay-data="{field:'SCJYDZ',width:'15%'}">生产经营地址</th>
	...
	<div id="addmain" style="display: none;">
		...
		<form class="layui-form" id="form3" action=""
			...
			<div class="layui-form-item">
				<label class="layui-form-label" readonly="readonly"  style="width: 150px;"><span style="color:red">*</span>纳税人识别码:</label>
				<div class="layui-input-inline">
	...
	function getData() {
		$.ajax({
			type : "post", //请求方式
			async : false, //是否异步
			url : "ajax.do?ctrl=ZjkcAction_doQuery&pageNo=" + pageNo + "&pageSize="
					+ pageSize,
			data : $("#form").serialize(),
			dataType : "json",
...
/* 	function formatDateTime(inputTime) {  
	...
	layui.use([ 'form' ], function() {
		...
		form.on('submit(doExcel)', function(data) {
			...
			window.location.href="export.do?ctrl=ZjkcAction_export&paramNsrmc="+paramNsrmc;
			pageNo = 1; //当点击搜索的时候，应该回到第一页
			return false; //阻止表单跳转。如果需要表单跳转，去掉这段即可。
		});
		...
		form.on('submit(addSave)', function(data) {
			$.ajax({
				type : "post", //请求方式
				async : false, //是否异步
				url : "ajax.do?ctrl=ZjkcAction_doQueryCount",
		...
		function addAjax(){
			$.ajax({
				type : "post", //请求方式
				async : false, //是否异步
				url : "ajax.do?ctrl=ZjkcAction_doAdd",
				data : $("#form3").serialize(),
				dataType : "json",
				success : function(obj) {
					if (obj != null && obj.code == '000') {
		...
		function deleteAndInsertAjax(){
			$.ajax({
				type : "post", //请求方式
				async : false, //是否异步
				url : "ajax.do?ctrl=ZjkcAction_doDeleteAndInsert",
				data : $("#form3").serialize(),
				dataType : "json",
				success : function(obj) {
					if (obj != null && obj.code == '000') {
		...
		form.on('submit(updateSave)', function(data) {
			$.ajax({
				type : "post", //请求方式
				async : false, //是否异步
				url : "ajax.do?ctrl=ZjkcAction_doQueryCount2",
		...
		function updateAjax(){
			$.ajax({
				type : "post", //请求方式
				async : false, //是否异步
				url : "ajax.do?ctrl=ZjkcAction_doUpdate",
				data : $("#form5").serialize(),
				dataType : "json",
				success : function(obj) {
					if (obj != null && obj.code == '000') {
			...
			if (layEvent === 'deleteInfo') { //删除
				...
				, function(index) {
					$.ajax({
						type : "post", //请求方式
						async : true, //是否异步
						url : "ajax.do?ctrl=ZjkcAction_doDel",
						data : data,
						dataType : "json",
						success : function(obj) {
							if (obj.code == '000') {
