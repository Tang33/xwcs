...
<body style="overflow-x: hidden">
		<blockquote class="layui-elem-quote layui-text">
			本功能用于对系统的“地税外区企业清册管理”进行操作管理！</blockquote>
				...
				<table class="layui-table" id="table" lay-filter="sj">
					<thead>
						<tr>
							...
							<th lay-data="{field:'SCJYDZ',width:'20%'}">生产经营地址</th>
							<th lay-data="{field:'JD_MC',width:'8%'}">所属街道</th>
	...
	function getData() {
		$.ajax({
			type : "post", //请求方式
			async : false, //是否异步
			url : "ajax.do?ctrl=DsWqAction_doQuery&pageNo=" + pageNo + "&pageSize="
					+ pageSize,
			data : $("#form").serialize(),
			dataType : "json",
...
/* 	function formatDateTime(inputTime) {  
	...
	layui.use([ 'form' ], function() {
		...
		form.on('submit(doExcel)', function(data) {
			...
			window.location.href="export.do?ctrl=DsWqAction_export&paramNsrmc="+paramNsrmc+"&paramJd="+paramJd;
			pageNo = 1; //当点击搜索的时候，应该回到第一页
			return false; //阻止表单跳转。如果需要表单跳转，去掉这段即可。
		});
		...
		form.on('submit(addSave)', function(data) {
			$.ajax({
				type : "post", //请求方式
				async : false, //是否异步
				url : "ajax.do?ctrl=DsWqAction_doQueryCount",
		...
		function addAjax(){
			$.ajax({
				type : "post", //请求方式
				async : false, //是否异步
				url : "ajax.do?ctrl=DsWqAction_doAdd",
				data : $("#form3").serialize(),
				dataType : "json",
				success : function(obj) {
					if (obj != null && obj.code == '000') {
		...
		function deleteAndInsertAjax(){
			$.ajax({
				type : "post", //请求方式
				async : false, //是否异步
				url : "ajax.do?ctrl=DsWqAction_doDeleteAndInsert",
				data : $("#form3").serialize(),
				dataType : "json",
				success : function(obj) {
					if (obj != null && obj.code == '000') {
		...
		form.on('submit(updateSave)', function(data) {
			$.ajax({
				type : "post", //请求方式
				async : false, //是否异步
				url : "ajax.do?ctrl=DsWqAction_doQueryCount2",
		...
		function updateAjax(){
			$.ajax({
				type : "post", //请求方式
				async : false, //是否异步
				url : "ajax.do?ctrl=DsWqAction_doUpdate",
				data : $("#form5").serialize(),
				dataType : "json",
				success : function(obj) {
					if (obj != null && obj.code == '000') {
			...
			if (layEvent === 'deleteInfo') { //删除
				...
				, function(index) {
					$.ajax({
						type : "post", //请求方式
						async : true, //是否异步
						url : "ajax.do?ctrl=DsWqAction_doDel",
						data : data,
						dataType : "json",
						success : function(obj) {
							if (obj.code == '000') {
