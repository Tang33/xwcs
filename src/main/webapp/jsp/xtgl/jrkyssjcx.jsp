...
var fz="";
	...
	function initfzzd() {
		$.ajax({
			type : "post", //请求方式
			async : true, //是否异步
			url:"ajax.do?ctrl=jrkyssjcx_queryfz",
			data: { 
		    },
			dataType : "json",
			success : function(obj) {
	...
	//查询页面加载下拉框中的数据
	 function queryList(){
		$.ajax({
			type : "post", //请求方式
			async : true, //是否异步
			url:"ajax.do?ctrl=jrkyssjcx_selectList",
			data: "",
			dataType : "json",
			success : function(obj) {
				var data = obj.data;
			 ...
			 */
			 if(text=="街道乡镇"||text=="征收项目"||text=="征收品目"||text=="行业大类"||text=="行业中类"||text=="行业门类"||text=="行业"||text=="登记注册类型"){
					...
					var str = "<div style='float: left;'><label style='line-height: 40px;'>"+text+"：</label></div><input type='hidden' id='"+xz+""+k+"' value='hid' >";
					 str += "<select xm-select='select"+k+"' id='"+xz+"_"+k+"'  xm-select-type='1' style='width: 150%;' lay-filter='columns"+k+"'></select></div>";
								...
								$.ajax({
									type : "post", //请求方式
									async : true, //是否异步
									url:"ajax.do?ctrl=jrkyssjcx_selectSZ",
									data: { 
								        "xz": xz
								    },
									dataType : "json",
					...
					$.ajax({
						type : "post", //请求方式
						async : true, //是否异步
						url:"ajax.do?ctrl=jrkyssjcx_queryzd",
						data: { 
					        "xz": xz
					    },
						dataType : "json",
					...
					if(text.indexOf("期")!=-1){
						...
						$.ajax({
							type : "post", //请求方式
							async : true, //是否异步
							url:"ajax.do?ctrl=jrkyssjcx_selectislike",
							data: { 
						        "xz": xz
						    },
							dataType : "json",
	...
	$("#selectfz").click(function() {
		 var form = $("#form1").serializeArray();
	    	...
	    	window.open(encodeURI("outSdbg3.do?drrq="+$("#drrq").val()+"&fz="+$("#fz").val()+"&form="+JSON.stringify(form)));
		...
		$("#chaxun").click(function() {
			...
			layui.use('table', function(){
		        ...
		        table.render({
		          ...
		          /* ,data: data //数据 */
		          ,method : 'post'
		          //,url:"ajax.do?ctrl=jrkyssjcx_querySdbg"
		          ,url:"jrkyssjcyController/jrkyssjquery.do"
		          ,cellMinWidth: 120
		          ,where: {
		        	  drrq:$("#drrq").val(),
