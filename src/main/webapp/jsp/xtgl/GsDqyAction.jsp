...
<body style="overflow-x: hidden">
		<blockquote class="layui-elem-quote layui-text">
			本功能用于对系统的“国税大企业清册管理”进行操作管理！</blockquote>
		<form class="layui-form" id="form" action="">
				...
				<div class="layui-colla-item">
					<div class="layui-colla-content  layui-show">
						<div class="layui-row">
							<div class="layui-col-md4">
								<div class="layui-inline">
									...
									<div class="layui-input-inline">
										<input id="paramNsrmc" name="paramNsrmc" type="text" placeholder="请输入纳税人名称" class="layui-input" style="width:160%">
									</div>
								</div>
							</div>
							<div class="layui-col-md4">
									...
									<div class="layui-row">
										<div class="layui-col-md12">
											...
											<div class="layui-input-block">
												<select id="paramJd" name="paramJd" lay-filter="paramJd">
							...
							<div class="layui-col-md4">
								<a class="layui-btn" id="button" lay-filter="button">查询</a>
			...
			<fieldset class="layui-elem-field layui-field-title" style="margin-top: 20px;">
				...
				<table class="layui-table" id="table" lay-filter="sj">
					<thead>
						<tr>
							<th lay-data="{field:'JD_DM',width:'10%',style:'display:none;'}">所属街道</th>
							<th lay-data="{field:'HY_DM',width:'10%',style:'display:none;'}">所属行业</th>
							<th lay-data="{field:'NSRSBH',width:'10%'}">纳税人识别码</th>
							<th lay-data="{field:'NSRMC',width:'20%'}">纳税人名称</th>
							<th lay-data="{field:'SCJYDZ',width:'10%'}">生产经营地址</th>
							<th lay-data="{field:'JD_MC',width:'10%'}">所属街道</th>
							<th lay-data="{field:'HYML_MC',width:'20%'}">所属行业</th>
							<th lay-data="{field:'RQ',width:'15%'}">最后变更日期</th>
							<th lay-data="{field:'dfkj',width:'10%',toolbar:'#bar'}">操作</th>
						</tr>
					</thead>
					<tbody id="ttbody">
					</tbody>
		...
		<form class="layui-form" id="form5" action=""
			...
			<div class="layui-form-item">
				<label class="layui-form-label" readonly="readonly"  style="width: 150px;">纳税人识别码:</label>
				<div class="layui-input-inline">
					<input type="text" id="update_NSRSBH" style="width: 250px;" readonly="readonly" name="update_NSRSBH" 
						class="layui-input">
				</div>
				<!-- <label class="layui-form-label" style="width: 150px;">所属行业:</label>
				<div class="layui-input-inline">
					<input type="text" id="update_HYML_MC" name="update_HYML_MC"  
						class="layui-input">
				</div> -->
				<div class="layui-input-inline" style="margin-left: 70px;width: 390px;">
					<label class="layui-form-label">所属行业</label>
					<div class="layui-input-block">
						<select id="update_HYML_MC" name="update_HYML_MC" lay-filter="paramJd">
						<option value="%">全部</option>
						 <c:forEach items="${hylist}" var="s">
						 	<option value="${s.HYML_DM}">${s.HYML_MC}</option>
					     </c:forEach>
						</select>
					</div>
				</div>
			...
			<div class="layui-form-item">
				...
				<div class="layui-input-inline" style="margin-left: 70px;width: 390px;">
					<label class="layui-form-label">所属街道</label>
					<div class="layui-input-block">
						<select id="update_JD_MC" name="update_JD_MC" lay-filter="paramJd">
						<option value="%">全部</option>
						 <c:forEach items="${jdlist}" var="s">
						 	<option value="${s.JD_DM}">${s.JD_MC}</option>
					     </c:forEach>
	...
	<div id="addmain" style="display: none;">
		...
		<form class="layui-form" id="form3" action=""
			...
			<div class="layui-form-item">
				...
				<label class="layui-form-label" style="width: 150px;"><span style="color:red">*</span>所属行业:</label>
				<div class="layui-input-inline">
					<select name="add_HYML_DM" id="add_HYML_DM" lay-verify="required"
							lay-search="" lay-filter="add_HYML_DM">
			...
			<div class="layui-form-item">
				...
				<label class="layui-form-label" style="width: 150px;"><span style="color:red">*</span>街道名称:</label>
				<div class="layui-input-inline">
	...
	<!-- <script>
		layui.use(['upload', 'form', 'laypage', 'layer'], function() {
			var $ = layui.jquery,
				form = layui.form,
	...
	function getData() {
		$.ajax({
			type : "post", //请求方式
			async : false, //是否异步
			url : "ajax.do?ctrl=GsDqyAction_doQuery&pageNo=" + pageNo + "&pageSize="
					+ pageSize,
			data : $("#form").serialize(),
			dataType : "json",
...
/* 	function formatDateTime(inputTime) {  
	...
	$('#paramNsrmc').bind('keydown',function(event){
	    if(event.keyCode == "13") {
	    	getData();
	    }
	}); 
	...
	layui.use([ 'form' ], function() {
		var form = layui.form;
		form.on('select(paramJd)',function(data){
			getData();
			
		});
		form.on('submit(doExcel)', function(data) {
			var paramJd=$("#paramJd").val();
			var paramNsrmc=$("#paramNsrmc").val();
			window.location.href="export.do?ctrl=GsDqyAction_export&paramNsrmc="+paramNsrmc+"&paramJd="+paramJd;
		...
		form.on('submit(addSave)', function(data) {
			$.ajax({
				type : "post", //请求方式
				async : false, //是否异步
				url : "ajax.do?ctrl=GsDqyAction_doQueryCount",
				...
				success : function(obj) {
					if (obj != null && obj.code == '000') {
						...
						layer.confirm('纳税人识别及纳税人名称已存在，是否覆盖？',{title:'温馨提示'},function(e){
							deleteAndInsertAjax();
							layer.close(e);
						});
					} else if(obj != null && obj.code == '002'){
						layer.confirm('纳税人识别号已存在，是否覆盖？',{title:'温馨提示'},function(e){
							deleteAndInsertAjax();
							layer.close(e);
						});
					} else if(obj != null && obj.code == '003'){
						layer.confirm('纳税人名称已存在，是否覆盖？',{title:'温馨提示'},function(e){
							deleteAndInsertAjax();
							layer.close(e);
						});
					}
		...
		function addAjax(){
			$.ajax({
				type : "post", //请求方式
				async : false, //是否异步
				url : "ajax.do?ctrl=GsDqyAction_doAdd",
				data : $("#form3").serialize(),
				dataType : "json",
				success : function(obj) {
					if (obj != null && obj.code == '000') {
		...
		function deleteAndInsertAjax(){
			$.ajax({
				type : "post", //请求方式
				async : false, //是否异步
				url : "ajax.do?ctrl=GsDqyAction_doDeleteAndInsert",
				data : $("#form3").serialize(),
				dataType : "json",
				success : function(obj) {
					if (obj != null && obj.code == '000') {
		...
		form.on('submit(updateSave)', function(data) {
			$.ajax({
				type : "post", //请求方式
				async : false, //是否异步
				url : "ajax.do?ctrl=GsDqyAction_doQueryCount2",
		...
		function updateAjax(){
			$.ajax({
				type : "post", //请求方式
				async : false, //是否异步
				url : "ajax.do?ctrl=GsDqyAction_doUpdate",
				data : $("#form5").serialize(),
				dataType : "json",
				success : function(obj) {
					if (obj != null && obj.code == '000') {
			...
			if (layEvent === 'deleteInfo') { //删除
				...
				, function(index) {
					$.ajax({
						type : "post", //请求方式
						async : true, //是否异步
						url : "ajax.do?ctrl=GsDqyAction_doDel",
						data : data,
						dataType : "json",
						success : function(obj) {
							if (obj.code == '000') {
				...
				editlayer = layer.open({
					...
					success : function() {
						...
						$("#update_JD_MC").val(data.JD_DM)
						$("#update_HYML_MC").val(data.HY_DM)
						layui.form.render();
					},
					cancel : function() {
						return;
