...
<body style="overflow-x: hidden">
		<blockquote class="layui-elem-quote layui-text">
			本功能用于对系统的“地税重点税源户清册管理”进行操作管理！</blockquote>
									...
									<div class="layui-inline">
										...
										<div class="layui-input-inline">
											<input id="paramNsrmc" name="paramNsrmc" type="text"
												placeholder="请输入纳税人名称" class="layui-input" style="width:160%;">
										</div>
									</div>
								</div>
		...
		<fieldset class="layui-elem-field layui-field-title" style="margin-top: 20px;">
				...
				<table class="layui-table" id="table" lay-filter="sj">
					<thead>
						<tr>
							<th lay-data="{field:'JD_DM',width:'10%',style:'display:none;width:10%'}">街道代码</th>
							<th lay-data="{field:'SWGLM',width:'10%'}">税务管理码</th>
							<th lay-data="{field:'NSRMC',width:'25%'}">纳税人名称</th>
							<th lay-data="{field:'SCJYDZ',width:'25%'}">生产经营地址</th>
							<th lay-data="{field:'JD_MC',width:'10%'}">所属街道</th>
							<th lay-data="{field:'RQ',width:'12%'}">最后变更日期</th>
							<th lay-data="{field:'dfkj',width:'20%',toolbar:'#bar'}">操作</th>
						</tr>
					</thead>
					<tbody id="ttbody">
		...
		<form class="layui-form" id="form5" action=""
			...
			<div class="layui-form-item">
				...
				<div class="layui-input-inline">
					<input type="text" id="update_SWGLM" style="width: 250px;" readonly="readonly" name="update_SWGLM" 
						class="layui-input">
				</div>
			...
			<div class="layui-form-item">
				...
				<div class="layui-input-inline" style="margin-left: 70px;width: 360px;">
					<label class="layui-form-label">所属街道:</label>
					<div class="layui-input-block">
	...
	function getData() {
		$.ajax({
			type : "post", //请求方式
			async : false, //是否异步
			url : "ajax.do?ctrl=DsZdsyhAction_doQuery&pageNo=" + pageNo + "&pageSize="
					+ pageSize,
			data : $("#form").serialize(),
			dataType : "json",
...
/* 	function formatDateTime(inputTime) {  
	...
	layui.use([ 'form' ], function() {
		...
		form.on('submit(doExcel)', function(data) {
			...
			window.location.href="export.do?ctrl=DsZdsyhAction_export&paramNsrmc="+paramNsrmc+"&paramJd="+paramJd;
			pageNo = 1; //当点击搜索的时候，应该回到第一页
			return false; //阻止表单跳转。如果需要表单跳转，去掉这段即可。
		});
		...
		form.on('submit(addSave)', function(data) {
			$.ajax({
				type : "post", //请求方式
				async : false, //是否异步
				url : "ajax.do?ctrl=DsZdsyhAction_doQueryCount",
				...
				success : function(obj) {
					if (obj != null && obj.code == '000') {
						...
						layer.confirm('税务管理码以及纳税人名称已存在，是否覆盖？',{title:'温馨提示'},function(e){
							deleteAndInsertAjax()
							layer.close(e);
						});
		...
		function addAjax(){
			$.ajax({
				type : "post", //请求方式
				async : false, //是否异步
				url : "ajax.do?ctrl=DsZdsyhAction_doAdd",
				data : $("#form3").serialize(),
				dataType : "json",
				success : function(obj) {
					if (obj != null && obj.code == '000') {
		...
		function deleteAndInsertAjax(){
			$.ajax({
				type : "post", //请求方式
				async : false, //是否异步
				url : "ajax.do?ctrl=DsZdsyhAction_doDeleteAndInsert",
				data : $("#form3").serialize(),
				dataType : "json",
				success : function(obj) {
					if (obj != null && obj.code == '000') {
		...
		form.on('submit(updateSave)', function(data) {
			$.ajax({
				type : "post", //请求方式
				async : false, //是否异步
				url : "ajax.do?ctrl=DsZdsyhAction_doQueryCount2",
		...
		function updateAjax(){
			$.ajax({
				type : "post", //请求方式
				async : false, //是否异步
				url : "ajax.do?ctrl=DsZdsyhAction_doUpdate",
				data : $("#form5").serialize(),
				dataType : "json",
				success : function(obj) {
					if (obj != null && obj.code == '000') {
			...
			if (layEvent === 'deleteInfo') { //删除
				...
				, function(index) {
					$.ajax({
						type : "post", //请求方式
						async : true, //是否异步
						url : "ajax.do?ctrl=DsZdsyhAction_doDel",
						data : data,
						dataType : "json",
						success : function(obj) {
							if (obj.code == '000') {
