...
<body style="overflow-x: hidden">
		<blockquote class="layui-elem-quote layui-text">
			本功能用于对系统的“国税外区企业清册管理”进行操作管理！</blockquote>
		<form class="layui-form" id="form" action="">
			...
			<fieldset class="layui-elem-field layui-field-title" style="margin-top: 20px;">
				...
				<table class="layui-table" id="table" lay-filter="sj">
					<thead>
						<tr>
							<th lay-data="{field:'JD_DM',style:'display:none;width:10%'}">街道代码</th>
							<th lay-data="{field:'NSRSBH',width:'10%'}">纳税人识别码</th>
							<th lay-data="{field:'NSRMC',width:'20%'}">纳税人名称</th>
							<th lay-data="{field:'SCJYDZ',width:'30%'}">生产经营地址</th>
							<th lay-data="{field:'JD_MC',width:'10%'}">所属街道</th>
							<th lay-data="{field:'RQ',width:'20%'}">最后变更日期</th>
	...
	function getData() {
		$.ajax({
			type : "post", //请求方式
			async : false, //是否异步
			url : "ajax.do?ctrl=GsWqAction_doQuery&pageNo=" + pageNo + "&pageSize="
					+ pageSize,
			data : $("#form").serialize(),
			dataType : "json",
...
/* 	function formatDateTime(inputTime) {  
	...
	layui.use([ 'form' ], function() {
		...
		form.on('submit(doExcel)', function(data) {
			...
			window.location.href="export.do?ctrl=GsWqAction_export&paramNsrmc="+paramNsrmc+"&paramJd="+paramJd;
			pageNo = 1; //当点击搜索的时候，应该回到第一页
		...
		form.on('submit(addSave)', function(data) {
			$.ajax({
				type : "post", //请求方式
				async : false, //是否异步
				url : "ajax.do?ctrl=GsWqAction_doQueryCount",
				...
				success : function(obj) {
					if (obj != null && obj.code == '000') {
						...
						layer.confirm('纳税人识别号以及纳税人名称已存在，是否覆盖？',{title:'温馨提示'},function(e){
							deleteAndInsertAjax()
							layer.close(e);
						});
		...
		function addAjax(){
			$.ajax({
				type : "post", //请求方式
				async : false, //是否异步
				url : "ajax.do?ctrl=GsWqAction_doAdd",
				data : $("#form3").serialize(),
				dataType : "json",
				success : function(obj) {
					if (obj != null && obj.code == '000') {
		...
		function deleteAndInsertAjax(){
			$.ajax({
				type : "post", //请求方式
				async : false, //是否异步
				url : "ajax.do?ctrl=GsWqAction_doDeleteAndInsert",
				data : $("#form3").serialize(),
				dataType : "json",
				success : function(obj) {
					if (obj != null && obj.code == '000') {
		...
		form.on('submit(updateSave)', function(data) {
			$.ajax({
				type : "post", //请求方式
				async : false, //是否异步
				url : "ajax.do?ctrl=GsWqAction_doQueryCount2",
		...
		function updateAjax(){
			$.ajax({
				type : "post", //请求方式
				async : false, //是否异步
				url : "ajax.do?ctrl=GsWqAction_doUpdate",
				data : $("#form5").serialize(),
				dataType : "json",
				success : function(obj) {
					if (obj != null && obj.code == '000') {
			...
			if (layEvent === 'deleteInfo') { //删除
				...
				, function(index) {
					$.ajax({
						type : "post", //请求方式
						async : true, //是否异步
						url : "ajax.do?ctrl=GsWqAction_doDel",
						data : data,
						dataType : "json",
						success : function(obj) {
							if (obj.code == '000') {
