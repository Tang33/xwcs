...
<body style="overflow-x: hidden">
		<blockquote class="layui-elem-quote layui-text">
			本功能用于对系统的“重点税源户管理”进行操作管理！</blockquote>
		...
		<form class="layui-form" id="form" action="">
						...
						<div class="layui-row">
							<div class="layui-row">
									...
									<div class="layui-col-md4">
										<label class="layui-form-label" style="width: 100px;">重点税源类别</label>
										<div class="layui-inline">
											<div class="layui-inline">
													<select id="paramlb" name="paramlb" lay-filter="paramlb">
													<!-- <option value="">全部</option> -->
													 <c:forEach items="${lblist}" var="s">
													 	<option value="${s.LB_DM}">${s.LB_MC}</option>
												     </c:forEach>
													</select>
												</div>
										</div>
			...
			<fieldset class="layui-elem-field layui-field-title" style="margin-top: 20px;">
				...
				<table class="layui-table" id="table" lay-filter="sj" lay-data="{id: 'table1'}">
					<thead>
						<tr>
							<th lay-data="{field:'JD_DM',width:'20%',style:'display:none;'}">街道代码</th>
							<th lay-data="{field:'LB_DM',width:'20%',style:'display:none;'}">列表代码</th>
							<th lay-data="{field:'NSRMC',width:'20%'}">纳税人名称</th>
							<th lay-data="{field:'LrRY_MC',width:'20%'}">建立者</th>
							<th lay-data="{field:'LRRQ',width:'20%'}">建立日期</th>
		...
		<form class="layui-form" id="form5" action=""
			...
			<div class="layui-form-item">
				<label class="layui-form-label" style="width: 150px;"><span style="color:red">*</span>纳税人名称:</label>
					<div class="layui-input-inline" style="width: 280px;">
						<input type="text" id="update_NSRMC" name="update_NSRMC"  lay-verify="required"
						 placeholder="请输入纳税人名称" class="layui-input">
				</div>
			</div>
			<div class="layui-form-item">
				<div class="layui-input-inline" style="margin-left: 70px;width: 390px;">
					<label class="layui-form-label"><span style="color:red">*</span>所属行业</label>
					<div class="layui-input-block">
						<select id="update_LB_DM" name="update_LB_DM" lay-filter="update_LB_DM">
						<option value="%">全部</option>
						 <c:forEach items="${lblist}" var="s">
						 	<option value="${s.LB_DM}">${s.LB_MC}</option>
					     </c:forEach>
			...
			<%-- <div class="layui-form-item">
				<div class="layui-input-inline" style="margin-left: 70px;width: 390px;">
					<label class="layui-form-label"><span style="color:red">*</span>所属街道</label>
					<div class="layui-input-block">
						<select id="update_JD_DM" name="update_JD_DM" lay-filter="add_JD_DM">
						<option value="%">全部</option>
		...
		<form class="layui-form" id="form3" action=""
			...
			<div class="layui-form-item">
				<div class="layui-input-inline" style="margin-left: 70px;width: 390px;">
					<label class="layui-form-label"><span style="color:red">*</span>所属行业</label>
					<div class="layui-input-block">
						<select id="add_LB_DM" name="add_LB_DM" lay-filter="add_LB_DM">
						<option value="%">全部</option>
						 <c:forEach items="${lblist}" var="s">
						 	<option value="${s.LB_DM}">${s.LB_MC}</option>
					     </c:forEach>
			...
			<div class="layui-form-item">
				<input type="hidden" id="add_oldLB_DM" name= "add_oldLB_DM" lay-filter="add_oldLB_DM">
				<input type="hidden" id="add_oldNSRMC" name= "add_oldNSRMC" lay-filter="add_oldNSRMC">
				
				<%-- <div class="layui-input-inline" style="margin-left: 70px;width: 390px;">
					<label class="layui-form-label"><span style="color:red">*</span>所属街道</label>
					<div class="layui-input-block">
						<select id="add_JD_DM" name="add_JD_DM" lay-filter="add_JD_DM">
						<option value="%">全部</option>
						 <c:forEach items="${jdlist}" var="s">
						 	<option value="${s.JD_DM}">${s.JD_MC}</option>
					     </c:forEach>
	...
	function getData() {
		$.ajax({
			type : "post", //请求方式
			async : false, //是否异步
			url : "ajax.do?ctrl=JdZdsyhAction_doQuery&pageNo=" + pageNo + "&pageSize="
					+ pageSize,
			data : $("#form").serialize(),
			dataType : "json",
	...
	layui.use([ 'form' ], function() {
		var form = layui.form;
		form.on('select(paramlb)',function(data){
			getData();
...
/* 	function formatDateTime(inputTime) {  
	...
	function getTbale(data) {
		...
		$.each(data, function(v, o) {
			...
			s += '<tr>';
			s += '<td>' + (o.JD_DM == undefined ? "-" : o.JD_DM)+ '</td>';
	...
	layui.use([ 'form' ], function() {
		...
		form.on('submit(doExcel)', function(data) {
			var paramlb=$("#paramlb").val();
			window.location.href="export.do?ctrl=JdZdsyhAction_export&paramlb="+paramlb;
		...
		form.on('submit(addSave)', function(data) {
			$.ajax({
				type : "post", //请求方式
				async : false, //是否异步
				url : "ajax.do?ctrl=JdZdsyhAction_doQueryCount",
		...
		function addAjax(){
			$.ajax({
				type : "post", //请求方式
				async : false, //是否异步
				url : "ajax.do?ctrl=JdZdsyhAction_doAdd",
				data : $("#form3").serialize(),
				dataType : "json",
				success : function(obj) {
					if (obj != null && obj.code == '000') {
		...
		function deleteAndInsertAjax(){
			$.ajax({
				type : "post", //请求方式
				async : false, //是否异步
				url : "ajax.do?ctrl=JdZdsyhAction_doDeleteAndInsert",
				data : $("#form3").serialize(),
				dataType : "json",
				success : function(obj) {
					if (obj != null && obj.code == '000') {
		...
		form.on('submit(updateSave)', function(data) {
			$.ajax({
				type : "post", //请求方式
				async : false, //是否异步
				url : "ajax.do?ctrl=JdZdsyhAction_doQueryCount2",
		...
		function updateAjax(){
			$.ajax({
				type : "post", //请求方式
				async : false, //是否异步
				url : "ajax.do?ctrl=JdZdsyhAction_doUpdate",
				data : $("#form5").serialize(),
				dataType : "json",
				success : function(obj) {
					if (obj != null && obj.code == '000') {
	...
	layui.use('table', function() {
		...
		table.on('tool(sj)', function(obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
			...
			/* if (layEvent === 'delShip') { //删除
				layer.confirm('真的删除船舶' + data.cb_name + '么?', function(index) {
					$.ajax({
						type : "post", //请求方式
						async : true, //是否异步
						url : "ajax.do?ctrl=shipManager_delship",
						data : data,
						dataType : "json",
						success : function(obj) {
							if (obj.code == '000') {
								layer.msg('删除船舶' + data.cb_name + ' 成功！');
							} else {
								layer.alert('删除船舶' + data.cb_name + '失败！');
							}
							layer.close(index);
							$("#button").click();
						},
