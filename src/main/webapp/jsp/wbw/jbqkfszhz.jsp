...
<body style="overflow-x: hidden">
		...
		<div class="layui-form" lay-filter="test1">
												...
												<div class="layui-col-md7">
													...
													<div class="layui-input-block">
														<select name="sortname" id="sortname" style="width: 40%;">
															<option value="ZSXM_DM">本年至本月累计数</option>
															<option value="HS">同比增减额</option>
															<option value="SSBNYLJ">同比增减率</option>
															<option value="SNSSTQS">上年全年合计</option>
															<option value="SSTBZJE">街道名称</option>
															<option value="SSTBZJL">户数</option>
														</select>
													</div>
												</div>
			...
			<fieldset class="layui-elem-field layui-field-title" style="margin-top: 20px;">
				...
				<table class="layui-table" id="table" lay-filter="user">
					<thead>
						<tr>
							<th lay-data="{field:'ROWNUM'}">序号</th>
							<th lay-data="{field:'ZSXM_MC'}">税种</th>
							<th lay-data="{field:'PX'}">本年截止当月税收</th>
							<th lay-data="{field:'URL'}">上年同期数</th>
							<th lay-data="{field:'YXBZ'}">同比增减额</th>
							<th lay-data="{field:'GNMC'}">同比增减率</th>
						</tr>
					</thead>
					<tbody id="ttbody">
					</tbody>
	...
	<script>
		layui.use(['form', 'layedit', 'laydate','laypage'], function() {
			...
			ajax({
				url:"ajax.do?ctrl=jbqkfszhz_queryInit",
				data:{},
				type:'post',
			...
			form.on('submit(export)',function(){
				...
				console.log(">>>执行导出<<<");
				window.location.href="export.do?ctrl=jbqkfszhz_export&cxrq="+cxrq
						+"&tjkj="+tjkj+"&jd_dm="+jd_dm+"&sortname="+sortname
						+"&sorttype="+sorttype+"&tjdw="+tjdw+"&hy_dm="+hy_dm+"&zsxm_dm="+zsxm_dm;
			});
		...
		function getData() {			
			...
			showLoad();
			ajax({
				url:"ajax.do?ctrl=jbqkfszhz_queryFszHzb",
				data:{
					"pageNo":pageNo,
					"pageSize":pageSize,
					"cxrq":cxrq,
	    ...
	    function getTbale(data) {
	       ...
	       $.each(data, function(v, o) {
	           s += '<tr><td>' + (v+1) + '</td>';
	           s += '<td>' + o.ZSXM_MC + '</td>';
	           s += '<td>' + (o.SSBNYLJ == null ? "0":o.SSBNYLJ ) + '</td>';
	           s += '<td>' + (o.SNSSTQS == null ? "0":o.SNSSTQS ) + '</td>';
	           s += '<td>' + (o.SSTBZJE == null ? "0":o.SSTBZJE ) + '</td>';
	           s += '<td>' + (o.SSTBZJL == "%" ? "0%":o.SSTBZJL ) + '</td>';
	           s += '</tr>';
	       });
	       $("#ttbody").html(s);
	       //执行渲染
