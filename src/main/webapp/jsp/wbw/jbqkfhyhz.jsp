...
<body style="overflow-x: hidden">
	<div class="layui-form" lay-filter="test1">
												...
												<div class="layui-col-md7">
													...
													<div class="layui-input-block">
														<select name="sortname" id="sortname" style="width: 40%;">
															<option value="SSBNYLJ">本年1-n累计数（税收）</option>
															<option value="SNSSTQS">上年同期数（税收）</option>
															<option value="SSTBZJE">同比增减额（税收）</option>
															<option value="SSTBZJL">同比增减率（税收）</option>
															<option value="SSSNQNHJ">上年全年合计（税收）</option>
															<option value="YSSRBYLJ">本年1-n累计数（营业收入）</option>
															<option value="YSSRSNTQS">上年同期数（营业收入）</option>
															<option value="YSSRTBZJE">同比增减额（营业收入）</option>
															<option value="YYSRTBZJL">同比增加率（营业收入）</option>
															<option value="YYSRSNQNHJ">上年全年合计（营业收入）</option>

															<option value="HYML_MC">行业门类名称</option>
															<option value="HS">户数</option>
															<option value="HSZB">户数占比</option>
															<option value="SSZB">税收占比</option>
														</select>
													</div>
												</div>
			...
			<fieldset class="layui-elem-field layui-field-title" style="margin-top: 20px;">
				...
				<table class="layui-table" id="table" lay-filter="user">
					<thead>
						<tr>
							<th rowspan="2" lay-data="{field:'xh'}">序号</th>
							<th rowspan="2" lay-data="{field:'LRRQ2'}">行业名称</th>
							<th rowspan="2" lay-data="{field:'LRRQ3'}">户数</th>
							<th rowspan="2" lay-data="{field:'LRRQ4'}">户数占比</th>
							<th colspan="4" lay-data="{field:'LRRQ5'}">营业收入情况</th>
							<th colspan="5" lay-data="{field:'LRRQ6'}">税收情况</th>
							<th rowspan="2" lay-data="{field:'YYSRZJLSSZJL'}">收入增减率/税收增减率</th>
						</tr>
						<tr>
							...
							<th lay-data="{field:'GNMC'}">增减额</th>
							<th lay-data="{field:'PX'}">增减率</th>
							<th lay-data="{field:'SJ_GNDM'}">税收占比</th>
							<th lay-data="{field:'URL'}">上年合计</th>
						</tr>
					</thead>
					<tbody id="ttbody">
	...
	<script>
		layui.use(['form', 'layedit', 'laydate','laypage'], function() {
			...
			ajax({
				url:"ajax.do?ctrl=jbqkfhyhz_queryInit",
				data:{},
				type:'post',
			...
			form.on('submit(export)',function(){
				...
				console.log(">>>执行导出<<<");
				window.location.href="export.do?ctrl=jbqkfhyhz_export&cxrq="+cxrq
		...
		function getData() {			
			...
			showLoad();
			ajax({
				url:"ajax.do?ctrl=jbqkfhyhz_queryHyHzb",
				data:{
					"pageNo":pageNo,
					"pageSize":pageSize,
					"cxrq":cxrq,
